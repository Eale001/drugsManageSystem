...
<div class="row" style="padding-top: 10px;">
	<div class="col-md-2">
		<h1 style="font-size: 24px; margin: 0;">评价查看</h1>
	</div>
	<div class="col-md-10 text-right">
		<a href="index"><span class="glyphicon glyphicon-home"></span> 首页</a>
		> <a disabled="disabled">评价查看</a>
	</div>
</div>
		...
		<div class="bgc-w box box-primary"
			...
			<div class="box-body no-padding chat-box">
				<div class="chat-title">
					<h4>
						<span>订单ID:${(comment.order.orderId)!''}</span>
					</h4>
                    <h4>
                        <span>订单车辆:${(comment.order.vehicle.vehicleName)!''}</span>
                    </h4>
                    <h4>
                        <span>订单开始时间:${(comment.order.beginTime)!''}</span>
                    </h4>
                    <h4>
                        <span>订单结束时间:${(comment.order.endTime)!''}</span>
                    </h4>
                    <h4>
                        <span>订单用户:${(comment.order.user.userName)!''}</span>
                    </h4>
				</div>
				<div class="chat-content" style="padding: 10px 40px 12px 16px;">
                    <h4>
                        <span>满意度:${(comment.satisfaction)!''}</span>
                    </h4>
					<p>${(comment.comment)!''}
					</p>
					<#--<#if notice.url!=''>-->
